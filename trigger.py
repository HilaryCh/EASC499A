...
class ShortLongTerms(object):
	...
	def output(self):
		...
		#print('n_enhancements',n_enhancements)
		#print('scales',self.scales)
		#print('l_windows',l_windows)
...
class Components(object):
	...
	def output(self):
		...
		#print('')
		#print('nscale',nscale)
		return channels, nscale, l_windows  #nscale+1
...
class Ratio(object):
	...
	def output(self):
		...
		print('self.enhancement_factor',self.enhancement_factor)
...
class Correlate(object):
	...
	def output(self):
		...
		self.pre_processed_data = pre_processed_data[0] # =channels
		self.enhancement_factor = pre_processed_data[1]
		self.l_windows = pre_processed_data[2]
		...
		for station_i, station_data in enumerate(self.pre_processed_data[0]): # =stations
			for enhancement_i, enhancement_data in enumerate(self.pre_processed_data[0][station_i]):
				...
				for channel_i in range(1,len(self.pre_processed_data)) :
					...
					if np.nansum(np.abs( self.pre_processed_data[channel_i][station_i][enhancement_i] )) > 0:
						...
						a = self.pre_processed_data[0][station_i][enhancement_i]  #Z
		...
		#print('self.l_windows',self.l_windows)
		print('self.enhancement_factor',self.enhancement_factor)
		cf[ cf< dtiny ] = dtiny
		#print('cf',cf)
